...
class CNN(object):
    def __init__(
      ...
      filter_sizes, num_filters, l2_reg_lambda=0.0):
        ...
        self.input_question = tf.placeholder(tf.int32, [batch_size, sequence_length], name="input_question")
        #正向答案
        self.input_r_answer = tf.placeholder(tf.int32, [batch_size, sequence_length], name="input_r_answer")
        #负向答案
        self.input_w_answer = tf.placeholder(tf.int32, [batch_size, sequence_length], name="input_w_answer")
        self.dropout_keep_prob = tf.placeholder(tf.float32, name="dropout_keep_prob")
        l2_loss = tf.constant(0.0)
        print("input_question ", self.input_question)

        with tf.device('/gpu:0'), tf.name_scope("embedding"):
            W = tf.Variable(
                tf.random_uniform([vocab_size, embedding_size], -1.0, 1.0),
                name="W")
            self.embedded_chars_1 = tf.nn.embedding_lookup(W, self.input_question)
            self.embedded_chars_2 = tf.nn.embedding_lookup(W, self.input_r_answer)
            self.embedded_chars_3 = tf.nn.embedding_lookup(W, self.input_w_answer)
        ...
        pooled_reshape_1 = tf.reshape(tf.concat(axis=3, values=pooled_outputs_1), [-1, num_filters_total]) 
        pooled_reshape_2 = tf.reshape(tf.concat(axis=3, values=pooled_outputs_2), [-1, num_filters_total]) 
        pooled_reshape_3 = tf.reshape(tf.concat(axis=3, values=pooled_outputs_3), [-1, num_filters_total]) 
        ...
        with tf.name_scope("output"):
            #计算向量夹角Batch模式
            self.cos_12 = tf.div(pooled_mul_12, tf.multiply(pooled_len_1, pooled_len_2), name="scores") 
